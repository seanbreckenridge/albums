...
def parse_command_line_args():
    ...
    parser.add_argument(
        "-n",
        type=int,
        help="Changes the number of albums to return." + " Default is 10.",
        default=10,
    )
    parser.add_argument(
        "-r",
        "--random",
        action="store_true",
        help="Chooses random albums instead " + "of listing chronologically.",
    )
    parser.add_argument(
        "-o",
        "--open",
    ...
    parser.add_argument(
        "-q", "--quiet", action="store_true", help="quiet mode - only print errors."
    )
    parser.add_argument(
        "-m",
        "--memory",
...
def get_credentials():
    ...
    if not os.path.exists(credential_dir):
        print(
            "Credentials have not been setup properly. Run setup_credentials.py",
            file=sys.stderr,
        )
        sys.exit(1)
...
if __name__ == "__main__":
    ...
    if open_from_memory:
        try:
            ...
            webbrowser.open(
                "https://docs.google.com/spreadsheets/d/{0}/edit#gid={1}&range=A{2}".format(
                    spreadsheet_id, pageId, prev_album_cell
                )
            )
    ...
    if choose_random:  # random choice
        ...
        if open_in_browser and not choose_random:
            webbrowser.open_new_tab(
                "https://docs.google.com/spreadsheets/d/{0}/edit#gid={1}&range=A{2}".format(
                    spreadsheet_id, pageId, link_range
                )
            )
    print(p_table)
